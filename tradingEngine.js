...
class TradingEngine {
  ...
  /** Change leverage for future orders. Updates local position-sizing config
   * and, when not in dry-run, also pushes the change to the exchange. */
  async setLeverage(leverage) {
    ...
    if (!this.config.dryRun) {
      const product = await this._getProduct();
      await this.client.setLeverage(product.id, lev);
    }
    this.config.leverage = lev;
    return lev;
  }

  /** Places an immediate market order bypassing the confluence signal —
   * triggered by the user from the dashboard, not the strategy. */
  async manualTrade({ side, riskPercent } = {}) {
    if (side !== 'buy' && side !== 'sell') {
      throw new Error("side must be 'buy' or 'sell'");
    }
    if (this.state.position) {
      throw new Error('A position is already open — close it before opening a new one (one at a time)');
    }
    if (this.state.tradingHalted) {
      throw new Error('Daily loss limit reached — trading is halted for today');
    }
    if (!this.state.indicators || this.state.price == null) {
      throw new Error('No market data yet — wait for the next tick and try again');
    }
    ...
    const effectiveRiskPercent = riskPercent != null && riskPercent > 0 ? riskPercent : this.config.riskPercent;
    const { size, capitalAtRisk } = calculatePositionSize({
    ...
    await this._placeEntryOrder({ side, size, product, stopLoss, takeProfit });
    ...
    this._recordTrade({
      ...
      takeProfit,
      capitalAtRisk: +capitalAtRisk.toFixed(2),
      reasons: ['Manual trade — placed from dashboard, not the confluence signal'],
      dryRun: this.config.dryRun,
      manual: true,
    });

    this.logger.log(`Manual ${side} ${size} contracts @ ${ind.price} | SL ${stopLoss} TP ${takeProfit}`);
    return this.state;
  }
